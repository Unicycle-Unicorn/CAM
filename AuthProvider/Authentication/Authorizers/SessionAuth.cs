...
public class SessionAuth : ICamAuthorizer
{
    ...
    public async Task<(AuthorizationResult, Action)> AuthenticateAsync(HttpContext context, ICamInterface cam)
    {
        ...
        if (CookieUtils.TryGetCookie(request, CookieUtils.Session, out string sessionId) && HeaderUtils.TryGetHeader(request, HeaderUtils.XAuthCSRF, out string csrf))
        {
            ...
            {
                AuthorizationResult result = await cam.AuthenticateSessionAsync(sessionId);
    ...
    public async Task<(AuthorizationResult, Action)> AuthorizeAsync(HttpContext context, ICamInterface cam, string permission)
    {
        ...
        if (CookieUtils.TryGetCookie(request, CookieUtils.Session, out string sessionId) && HeaderUtils.TryGetHeader(request, HeaderUtils.XAuthCSRF, out string csrf))
        {
            if (CSRFUtils.VerifyCSRF(sessionId, csrf))
            {
                AuthorizationResult result = await cam.AuthorizeSessionAsync(sessionId, permission);
                return (result, AddItemsAction(context, result, sessionId));
            }
        }
